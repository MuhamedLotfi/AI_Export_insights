...
# The multilingual embedding model (BAAI/bge-m3) handles Arabic natively,
# so the separate TranslateGemma translation model is no longer needed.
# This file is kept for future reference if translation is needed again.
# =============================================================================
...
# import ollama
#
# from backend.config import AI_CONFIG, DATA_DIR
# import os
# import json
#
...
#         self.config_path = os.path.join(DATA_DIR, "ai_config.json")
#         self.enabled = self._load_enabled_state()
#         logger.info(f"TranslationService initialized with model: {self.model}, enabled: {self.enabled}")
#
...
#                         return config_data["translation_enabled"]
#         except Exception as e:
#             logger.error(f"[TRANSLATION] Error loading config: {e}")
#         return AI_CONFIG.get("translation_enabled", True)
#
...
#         """Enable or disable translation at runtime and persist"""
#         self.enabled = enabled
#         logger.info(f"[TRANSLATION] Translation service {'enabled' if enabled else 'disabled'}")
#
#         try:
#             config_data = {}
#             if os.path.exists(self.config_path):
...
#         except Exception as e:
#             logger.error(f"[TRANSLATION] Error saving config: {e}")
#
